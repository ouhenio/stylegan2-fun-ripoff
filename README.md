...
the second network you have trained, the Discriminator. The following is a *starting code*, meant to inspire more usage of
  this network. Currently, there are three options available now:
  * *Discriminator Dream*: Inspired by [DeepDream](https://ai.googleblog.com/2015/06/inceptionism-going-deeper-into-neural.html), 
    we use our Discriminator instead of VGG. A starting image can be used, or a seed that will generate a random image. 
    Check available options via `python discriminator_synthesis.py dream --help`
  ...
  * *Visual reactive interpolation* (unfinished, first step in code): Pass in a video file, and we will use the Discriminator, VGG16,
    or [CLIP](https://github.com/openai/CLIP) (must be installed) as an encoder of each frame into the latent space `Z`, which we then pass
    to the Generator. As it is still a work in progress, the resulting video is not that impressive with the Discriminator,
    as all the frames are being mapped to the same area in `Z`, so these fake latents should be better fine-tuned. Check 
    available options with `python discriminator_synthesis.py visual-reactive --help`.
  * *Style transfer*: **TODO** Nothing here yet, will be added if the results are interesting.
...
still have the video). Likewise, for the moment, we are using [`scikit-video`](http://www.scikit-video.org/stable/) for 
opening videos in the visual reactive interpolation. Install it via `pip3 install scikit-video`.
...
* [ ] Add more options for generating images using the Discriminator (Cellular Automata, use more than one layer, etc.)
